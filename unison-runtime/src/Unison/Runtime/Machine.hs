...
exec _ !denv !_activeThreads !stk !k _ (Capture p) = do
  ...
  poke stk cap
  pure (False, denv, stk, k)
exec env !denv !_activeThreads !stk !k _ (Prim1 MISS i)
...
exec env !denv !_activeThreads !stk !k _ (Prim1 CACH i)
  ...
  | otherwise = do
      ...
      pure (False, denv, stk, k)
exec env !denv !_activeThreads !stk !k _ (Prim1 CVLD i)
...
exec env !denv !_activeThreads !stk !k _ (Prim1 VALU i) = do
  ...
  pure (False, denv, stk, k)
exec env !denv !_activeThreads !stk !k _ (Prim1 DBTX i)
  | sandboxed env =
...
exec env !denv !_activeThreads !stk !k _ (PrimXX SDBX i j) = do
  ...
  sdbx env stk s c
  pure (False, denv, stk, k)
exec env !denv !_activeThreads !stk !k _ (PrimXX SDBV i j) = do
  s <- peekOffS stk i
  v <- peekOffBi stk j
  stk <- bump stk
  sdbv env stk s v
...
--   pure (False, denv, stk, k)
-- exec _ !denv !_activeThreads !stk !k _ (PrimXX CMPU i j) = do
--   x <- peekOff stk i
--   y <- peekOff stk j
--   stk <- bump stk
--   pokeI stk . pred . fromEnum $ universalCompare compare x y
--   pure (False, denv, stk, k)
exec _ !_ !_activeThreads !stk !k r (PrimXX THRO i j) = do
  name <- peekOffBi @Util.Text.Text stk i
  x <- peekOff stk j
  thro k r name x
exec env !denv !_activeThreads !stk !k _ (PrimXX TRCE i j) = do
  tx <- peekOffBi stk i
  clo <- peekOff stk j
  (False, denv, stk, k) <$ trce env tx clo
...
exec _ !denv !_trackThreads !stk !k _ (PrimIX op t n j) = do
  ...
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimXI op i t n) = do
  stk <- primxi stk op i t n
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimDX op d j) = do
  stk <- primdx stk op d j
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimXD op i d) = do
  stk <- primxd stk op i d
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimTX op t j) = do
  stk <- primtx stk op t j
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimXT op i t) = do
  stk <- primxt stk op i t
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimMX op m j) = do
  stk <- primmx stk op m j
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimXM op i m) = do
  stk <- primxm stk op i m
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimYX op y j) = do
  stk <- primyx stk op y j
  pure (False, denv, stk, k)
exec _ !denv !_trackThreads !stk !k _ (PrimXY op i y) = do
  stk <- primxy stk op i y
  pure (False, denv, stk, k)
...
sdbx :: CCache -> Stack -> USeq -> Closure -> IO ()
sdbx env stk s c = do
  l <- decodeSandboxArgument s
  b <- checkSandboxing env l c
  pokeBool stk $ b

sdbv :: CCache -> Stack -> USeq -> ANF.Value -> IO ()
sdbv env stk s v
  | sandboxed env = sandboxExn "Value.validateSandboxed"
  | otherwise = do
      l <- decodeSandboxArgument s
...
thro :: K -> Reference -> Util.Text.Text -> Val -> IO a
thro k r name x = do
  () <- throwIO (BU (traceK r k) (Util.Text.toText name) x)
  error "throwIO should never return"
{-# inline thro #-}

trce :: CCache -> Util.Text.Text -> Val -> IO ()
trce env tx clo
  | sandboxed env = sandboxExn "trace"
  | otherwise = case tracer env True clo of
      NoTrace -> pure ()
      SimpleTrace str -> do
...
sandboxExn :: String -> IO a
sandboxExn name = die $ "attempted to use sandboxed operation: " ++ name
...
eval env !denv !activeThreads !stk !k _ (Yield args)
  | asize stk > 0,
    VArg1 i <- args =
      peekArg stk i >>= apply env denv activeThreads stk k False ZArgs
  | otherwise = do
    ...
    (exception, denv, stk, k)
      ...
      | exception -> case EC.lookup TT.exceptionTag denv of
        Just eh -> do
          ...
          apply env denv activeThreads stk kk False (VArg1 (Ix 0)) eh
...
yield env !denv !activeThreads !stk !k = leap denv k
  where
    leap !denv0 (Mark a ps cs k) = do
      ...
      apply env denv activeThreads stk k False (VArg1 (Ix 0)) val
